...
int main(void)
{
    ...
    // cBPF filter (tcpdump -b -dd 'ip or arp')
    struct sock_filter cbpf_bytecode[] = {
    { 0x28, 0, 0, 0x0000000c },
    { 0x15, 1, 0, 0x00000800 },
    { 0x15, 0, 1, 0x00000806 },
    { 0x6,  0, 0, 0x00040000 },
    ...
    struct sock_fprog cbpf_filter;
    cbpf_filter.filter = cbpf_bytecode;
    cbpf_filter.len = sizeof(cbpf_bytecode) / sizeof(cbpf_bytecode[0]);

    cord_filter_type_t filter_type = CBPF_FILTER;
    cord_retval = CORD_FLOW_POINT_ATTACH_FILTER(cord_app_context.l2_eth_a, (void *)&cbpf_filter, (void *)&filter_type);
