...
static void cord_app_cleanup(void)
{
    ...
    CORD_DESTROY_FLOW_POINT(cord_app_context.l4_sctp);
...
int main(void)
{
    ...
    cord_app_context.l4_sctp = CORD_CREATE_L4_SCTP_FLOW_POINT('B', inet_addr(TEP_SOURCE_IP), inet_addr(TEP_DEST_IP), TEP_SOURCE_PORT, TEP_DEST_PORT, TRUE);
    ...
    cord_retval = CORD_EVENT_HANDLER_REGISTER_FLOW_POINT(cord_app_context.evh, cord_app_context.l4_sctp);
    ...
    while (1)
    {
        ...
        for (uint8_t n = 0; n < nb_fds; n++)
        {
            ...
            if (current_fd == cord_app_context.l2_eth->io_handle)
            {
                ...
                if (rx_bytes < sizeof(cord_eth_hdr_t) + iphdr_len + sizeof(cord_sctp_hdr_t))
                    continue; // Too short for SCTP header
                ...
                if (cord_compare_ipv4_dst_subnet_ntohl(ip, cord_ntohl(prefix_ip.s_addr), cord_ntohl(netmask.s_addr)))
                {
                    ...
                    cord_retval = CORD_FLOW_POINT_TX(cord_app_context.l4_sctp, 0, ip, total_len, &tx_bytes);
                    if (cord_retval != CORD_OK)
                    {
                        // Handle the error
                    }
            ...
            if (current_fd == cord_app_context.l4_sctp->aux_handles[CLIENT_CONN_AUX_HANDLE_INDEX])
            {
                cord_retval = CORD_FLOW_POINT_RX(cord_app_context.l4_sctp, 0, buffer, BUFFER_SIZE, &rx_bytes);
                if (cord_retval != CORD_OK)
                {
            ...
            if (current_fd == cord_app_context.l4_sctp->io_handle)
            {
                ...
                {
                    cord_retval = CORD_EVENT_HANDLER_REGISTER_AUX_HANDLE(cord_app_context.evh, cord_app_context.l4_sctp, CLIENT_CONN_AUX_HANDLE_INDEX);
                    if (cord_retval == CORD_OK)
                    {
