# L2 Virtual Patch (with DPDK, multi-thread)
...
```bash
cd ..
cd apps/l2_dpdk_patch_workers/test_deployment/
sudo ./deploy.sh
```
...
64 bytes from 172.16.111.1: seq=0 ttl=64 time=0.067 ms
64 bytes from 172.16.111.1: seq=1 ttl=64 time=0.036 ms
64 bytes from 172.16.111.1: seq=2 ttl=64 time=0.040 ms
64 bytes from 172.16.111.1: seq=3 ttl=64 time=0.040 ms
```
...
Close all container shells. On the host, inside the l2_dpdk_patch_workers/test_deployment directory, execute:
```bash
sudo ./cleanup.sh
```