...
{
    cord_app_cleanup();
    CORD_LOG("[CordApp] Terminating the PacketCord Patch Dot1Q App!\n");
...
int main(void)
{
    ...
    CORD_LOG("[CordApp] Launching the PacketCord Patch Dot1Q App!\n");
    ...
    while (1)
    {
        ...
        for (uint8_t n = 0; n < nb_fds; n++)
        {
            ...
            if (cord_app_context.evh->events[n].data.fd == cord_app_context.l2_eth_a->io_handle)
            {
                ...
                // Push a Dot1Q VLAN tag
                cord_retval = cord_push_vlan(&pkt_desc, 111, 0, 0, CORD_ETH_P_8021Q);
                if (cord_retval != CORD_OK)
                {
                    CORD_ERROR("[CordApp] Error: VLAN push");
                }

                // Pop the Dot1Q VLAN tag
                cord_retval = cord_pop_vlan(&pkt_desc);
                if (cord_retval != CORD_OK)
                {
                    CORD_ERROR("[CordApp] Error: VLAN pop");
                }
